...
//    private List<TimerHandler> mShineTimers;
    ...
    private void loadSharedPreference() {
        ...
        mDropType = PadBoardAI.getType(ComboMasterApplication.getsInstance().getDropOrbs());
    }
...
//        	callback.onCastFinish(true);
        ...
        case ST_COLOR_CHANGE: {
            ...
            //setChangeColorBoard(PadBoardAI.colorChangedBoard(gameBoard, data), callback);
            setChangeColorBoard(PadBoardAI.colorChangedBoard(gameBoard, data), data, callback, byEnemy);
            return true;
        }
...
//    };
    ...
    public void clearBoard(final ICastCallback callback) {
        ...
        int[][] newBoard = PadBoardAI.generateBoard(mDropType, false); //PadBoardAI.getRandomBoard();//
...
//                    mStack.push(gameBoard);
    ...
    private void swapOrb(int nx, int ny, int cx, int cy) {
        final float toX = cx * SimulatorConstants.ORB_SIZE + SimulatorConstants.OFFSET_X;
        final float toY = cy * SimulatorConstants.ORB_SIZE + SimulatorConstants.OFFSET_Y;
        float fromX = nx * SimulatorConstants.ORB_SIZE + SimulatorConstants.OFFSET_X;
        float fromY = ny * SimulatorConstants.ORB_SIZE + SimulatorConstants.OFFSET_Y;
    ...
    private void setSuperDarkOrb(ResourceManager res, Sprite s, int turns) {
        ...
        IEntity child = s.getChildByTag(TAG_SUPER_DARK);
        if (isBlack(s.getColor()) && child != null) {
            Text text = (Text) child.getChildByTag(TAG_SUPER_DARK_TEXT);
...
//					}
        ...
        IEntityModifier modifier = new ScaleModifier(0.6f, 1f, 28f) {
            ...
            protected void onModifierFinished(IEntity pItem) {
                ...
                if ( isTarget ) {
                    ...
                    } else if (data.get(size) == 99) {
                        int count = RandomUtil.range(data.get(1), data.get(2));
                        for (int i=0; i<count; ++i) {
                            int x = RandomUtil.getInt(PadBoardAI.ROWS);
                            int y = RandomUtil.getInt(PadBoardAI.COLS);
                            Sprite s = sprites[x][y];
                            setSuperDarkOrb(res, s, turns);
                        }
                    } else {
                ...
                for(int index=0; index<loop; index+=2) {
                    ...
                    if (pos < 3 || pos == 5 || pos == 6) {
                        ...
                        for (int i = 0; i < PadBoardAI.ROWS; ++i) {
                            ...
                            int x = SimulatorConstants.ORB_SIZE * i;
                            int addition = 0;
                            if(gameBoard[i][line] >= 10 && gameBoard[i][line] != PadBoardAI.X_ORBS
                            		&& toOrb<=5) {
                            	addition = 10;
                            }
                        	...
                        	if( (counterSpell[0] && isPoison(toOrb)) ||
                        		...
                        		ns.registerEntityModifier(new ScaleModifier(SimulatorConstants.SECOND_REFRESH, 0.4f, 2.0f){
                        			protected void onModifierFinished(IEntity pItem) {
                        				...
                        				s.registerEntityModifier(new ScaleModifier(SimulatorConstants.SECOND_REFRESH, 2.4f, 1.0f));
                        			};
                        		});
                        	} else {
                        ...
                        for (int j = 0; j < PadBoardAI.COLS; ++j) {
                            ...
                            int y = SimulatorConstants.ORB_SIZE * j;
                            int addition = (toOrb<=5 && gameBoard[line][j] >= 10 && gameBoard[line][j] != PadBoardAI.X_ORBS) ? 10
                                    : 0;
    ...
    public String getSceneName() {
        return PadGameScene.class.getSimpleName();
    }
