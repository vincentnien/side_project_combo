...
public class MultiplePadGameScene7x6 extends PlaygroundGameScene implements
	...
	private List<Double> getPoisonDamage(List<Match7x6> matches) {
		int size = matches.size();
		List<Double> list = new ArrayList<Double>();
		for (int i = 0; i < size; ++i) {
			list.add(DamageCalculator7x6.calculatePoison(
	...
	private List<Double> getRecovery(List<Match7x6> matches) {
		Map<String, Object> settings = new HashMap<String, Object>();
		settings.put("nullAwoken", isNullAwoken());
		int size = matches.size();
		List<Double> list = new ArrayList<Double>();
		for (int i = 0; i < size; ++i) {
			list.add(DamageCalculator7x6.calculateRecovery(
					mEnvironment.mTeamData.team(), matches.subList(0, i + 1),
					null, settings, ComboMasterApplication.getsInstance().isCopMode()));
		}
		return list;
	}

	private List<Double> getFactor(List<Match7x6> matches) {
		int size = matches.size();
		List<Double> list = new ArrayList<Double>();
		...
		for (int i = 0; i < size; ++i) {
			...
			if (!mEnvironment.mMemberList[currentTeam][0].isBinded()) {
				f1 = DamageCalculator7x6.getLeaderFactor(team,
			...
			if (!mEnvironment.mMemberList[currentTeam][5].isBinded()) {
				f2 = DamageCalculator7x6.getLeaderFactor(team,
	...
	private void adjustGameBoard(MatchBoard7x6 board) {
		...
		if (dropList.size() > 0) {
			for (Pair<RowCol, RowCol> pair : dropList) {
				...
				int fromY = orb.col * SimulatorConstants.ORB_SIZE_7x6
						+ SimulatorConstants.OFFSET_Y;
				int toY = target.col * SimulatorConstants.ORB_SIZE_7x6
						+ SimulatorConstants.OFFSET_Y;
				...
				if (PadBoardAI7x6.isValid(orb.row, orb.col)) {
					...
					newSprite.setPosition(target.row
							* SimulatorConstants.ORB_SIZE_7x6
	...
	private void loadSharedPreference() {
		...
		mDropType = PadBoardAI7x6.getType(ComboMasterApplication.getsInstance()
				.getDropOrbs());
	}
	...
	public boolean skillFired(ActiveSkill skill, ICastCallback callback,
		...
		case ST_TARGET_RANDOM:
			if (mCurrentState != GameState.GAME_END) {
				...
				setChangeColorBoard(PadBoardAI7x6.randomChangedBoard(board, set,
		...
		case ST_ADD_PLUS: {
			...
			setChangeColorBoard(
					PadBoardAI7x6.colorChangedBoard(gameBoard, addPlus), callback,
					false);
			return true;
		}
		...
		case ST_ADD_LOCK:
			if (mCurrentState != GameState.GAME_END) {
				...
				setChangeColorBoard(
						PadBoardAI7x6.colorChangedBoard(board, addPlus), callback,
						byEnemy);
			}
			return true;
...
//			callback.onCastFinish(true);
		...
		case ST_COLOR_CHANGE: {
			...
			// setChangeColorBoard(PadBoardAI7x6.colorChangedBoard(gameBoard,
			// data), callback);
			setChangeColorBoard(PadBoardAI7x6.colorChangedBoard(gameBoard, data),
			...
			if (listSize >= 0) {
				...
				for (int i = 0; i < csize; ++i) {
					...
					for (int j = 0; j < ccsize; ++j) {
						...
						int addition = (gameBoard[rc.row][rc.col] >= 10 && gameBoard[rc.row][rc.col] != PadBoardAI7x6.X_ORBS) ? 10
								: 0;
						int cv = color;
						if (cv <= 5) {
							cv += addition;
		...
		case ST_TRANSFORM:
			...
			for (int i = 0; i < PadBoardAI7x6.ROWS; ++i) {
				for (int j = 0; j < PadBoardAI7x6.COLS; ++j) {
					...
					// } else
					if (orb >= 10 && orb < 20 && orb != PadBoardAI7x6.X_ORBS
							&& newboard[i][j] <= 5) {
						newboard[i][j] += 10;
					}
				}
...
//				Pair<Integer, Integer> one = valid.get(rnd);
	...
	public void clearBoard(final ICastCallback callback) {
		...
		int[][] newBoard = PadBoardAI7x6.generateBoard(mDropType, false);// generateBoard(mDropType,
																		// false);
		...
		for (int row = 0; row < PadBoardAI7x6.ROWS; ++row) {
			for (int k = PadBoardAI7x6.COLS-1; k >= 0; --k) {
				...
				dropList.add(new Pair<RowCol, RowCol>(RowCol.make(row, k),
						RowCol.make(row, k - PadBoardAI7x6.COLS)));
			}
		}
		for (Pair<RowCol, RowCol> pair : dropList) {
			final RowCol target = pair.first;
	...
	private void generateAllBoard() {
		...
		for (int row = 0; row < PadBoardAI7x6.ROWS; ++row) {
			for (int k = 4; k >= 0; --k) {
				...
				int neworb = PadBoardAI7x6.getNewOrbRestrictChances(mDropType,
						null);
				if (neworb <= 5
						&& RandomUtil
								.getLuck(mPlusOrbCount[currentTeam][neworb] * 20)) {
	...
	List<Match7x6> matches = new ArrayList<Match7x6>();
	List<Double> factors;
	List<Double> heals;
			...
			handler.post(new Runnable() {
				...
				public void run() {
					...
					if (matches.size() == 0 && mAddComboSkill != null) {
						...
						for (int i = 0; i < addition; ++i) {
							matches.add(Match7x6.make(9, 3, 0, list));
						}
						mCombo.set(addition);
	...
	private void swapOrb(int nx, int ny, int cx, int cy) {
		final float toX = cx * SimulatorConstants.ORB_SIZE_7x6
				+ SimulatorConstants.OFFSET_X;
		final float toY = cy * SimulatorConstants.ORB_SIZE_7x6
				+ SimulatorConstants.OFFSET_Y;
		float fromX = nx * SimulatorConstants.ORB_SIZE_7x6
				+ SimulatorConstants.OFFSET_X;
		float fromY = ny * SimulatorConstants.ORB_SIZE_7x6
				+ SimulatorConstants.OFFSET_Y;
...
//					}
			...
			protected void onModifierFinished(IEntity pItem) {
				...
				if (isTarget) {
					...
					if (data.get(size) == -1) { // all
						...
						for (int i=0; i<count; ++i) {
							int x = RandomUtil.getInt(PadBoardAI7x6.ROWS);
							int y = RandomUtil.getInt(PadBoardAI7x6.COLS);
							Sprite s = sprites[x][y];
							setSuperDarkOrb(res, s, turns);
						}
					} else {
					...
					if (pos < 3 || pos == 5 || pos == 6) {
						...
						for (int i = 0; i < PadBoardAI7x6.ROWS; ++i) {
							...
							if (gameBoard[i][line] >= 10
									&& gameBoard[i][line] != PadBoardAI7x6.X_ORBS
									&& toOrb <= 5) {
								addition = 10;
							}
						...
						for (int j = 0; j < PadBoardAI7x6.COLS; ++j) {
							...
							int addition = (toOrb <= 5
									&& gameBoard[line][j] >= 10 && gameBoard[line][j] != PadBoardAI7x6.X_ORBS) ? 10
									: 0;
	...
	public String getSceneName() {
		return MultiplePadGameScene7x6.class.getSimpleName();
	}
				...
				Observable.from(mEnvironment.getEnemies())
						...
						.subscribe(new Action1<Enemy>() {
							...
							public void call(Enemy enemy) {
								if (!enemy.dead()) {
									DamageCalculator.AttackValue attack = new DamageCalculator.AttackValue(
											5000000, 0, 0);
