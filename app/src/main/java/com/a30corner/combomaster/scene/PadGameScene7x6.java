...
public class PadGameScene7x6 extends PlaygroundGameScene implements
        IOnMenuItemClickListener {

    public static final int TAG_SUPER_DARK = 99;
    public static final int TAG_SUPER_DARK_TEXT = 98;
...
//    private BoardStack mStack;
    ...
    private PadEnvironment mEnvironment;
    ...
    private PadGameScene.GameState mCurrentState = PadGameScene.GameState.GAME_INIT;
    ...
    private boolean forceNoDrop = false;
...
//    private List<TimerHandler> mShineTimers;
    ...
    private List<Double> getPoisonDamage(List<Match> matches) {
    	...
    	for(int i=0; i<size; ++i) {
    		list.add(DamageCalculator.calculatePoison(mEnvironment.mTeamData.team(), matches.subList(0, i+1), null));
    	}
    	return list;
    }
    ...
    private List<Double> getRecovery(List<Match> matches) {
    	...
    	List<Double> list = new ArrayList<Double>();
    	for(int i=0; i<size; ++i) {
    		list.add(DamageCalculator.calculateRecovery(mEnvironment.mTeamData.team(), matches.subList(0, i+1), null, settings));
    	}
    	return list;
    }
    ...
    private List<Double> getFactor(List<Match> matches) {
        ...
        for(int i=0; i<6; ++i) {
        	if (mEnvironment.mMemberList[i] != null) {
        		bindStatus.add(mEnvironment.mMemberList[i].isBinded());
        	} else {
        ...
        settings.put("hp", 100 * mEnvironment.mTeamData.getCurrentHp() / (float)mEnvironment.mTeamData.getHp());
    	for(int i=0; i<size; ++i) {
	        double f1 = 1.0;
	        if(!mEnvironment.mMemberList[0].isBinded()) {
	        	f1 = DamageCalculator.getLeaderFactor(team, mEnvironment.mMemberList[0].info, MonsterInfo.empty(0), matches.subList(0, i+1), settings, true);
	        }
	        double f2 = 1.0;
	        if(!mEnvironment.mMemberList[5].isBinded()) {
	        	f2 = DamageCalculator.getLeaderFactor(team, mEnvironment.mMemberList[5].info, MonsterInfo.empty(0), matches.subList(0, i+1), settings, true);
	        }
	        list.add(f1*f2*crossFactor);
    	}
        return list;
    }
    ...
    private void doRemoval(final MatchBoard7x6 board, final int current) {
        if (board.matches.size() == current) {
            ...
            if (comboCount <= mComboText.size()) {
                ...
                text.setZIndex(6);
                mComboText.add(text);
                factor = new Text(0f, 0f, ResourceManager.getInstance().getFontStroke(), "x10000.00 ", vbom);
            ...
            double poison = poisoned.get(comboCount-1) + bombed * 0.2 * mEnvironment.getTeam().getHp();
            if(poison>0.0) {
            ...
            if (r > 0.0) {
            	...
            	mHeals.setPosition((GameActivity.SCREEN_WIDTH-mHeals.getWidth())/2, y);
    ...
    private void adjustGameBoard(MatchBoard7x6 board) {
            ...
            for (int i = 0; i < PadBoardAI7x6.ROWS; ++i) {
                ...
                if (target != null && orb != null) {
                    ...
                    for (int k = target.col; k >= 0; --k) {
                        ...
                        if (mDropOnlySkill == null) {
                            neworb = PadBoardAI.getNewOrbRestrictChances(mDropType, dropData);
                        } else {
                            neworb = PadBoardAI.getNewOrb(mDropOnlySkill.getData());
                        }
                        if (neworb <= 5
                                && RandomUtil
                                .getLuck(mPlusOrbCount[neworb] * 20 + addition)) {
                            neworb += 10;
                        }
                        if (lockPercent > 0) {
        ...
        if (dropList.size() > 0) {
            for (Pair<RowCol, RowCol> pair : dropList) {
                ...
                int fromY = orb.col * SimulatorConstants.ORB_SIZE_7x6 + SimulatorConstants.OFFSET_Y;
                int toY = target.col * SimulatorConstants.ORB_SIZE_7x6 + SimulatorConstants.OFFSET_Y;
                ...
                if (PadBoardAI7x6.isValid(orb.row, orb.col)) {
                    ...
                    newSprite.setPosition(target.row * SimulatorConstants.ORB_SIZE_7x6
        ...
        for(LeaderSkill s : ls) {
            ...
            if(s.getType() == LeaderSkillType.LST_COMBO_ATTACK) {
                ...
                if(colorSkill != null) {
                    ...
                    if(f>1.0 || DamageCalculator.isLSMatched(mTeam,s,m,mScoreBoard,null,true)) {
                        return DamageCalculator.getLSCombo(s);
                    }
                }
            }
        ...
        while (hasChanged) {
            ...
            for (int i = 0; i < PadBoardAI7x6.ROWS; ++i) {
                ...
                if (target != null && orb != null) {
                    ...
                    int emptyLen = target.col + 1;
                    for (int k = target.col; k >= 0; --k) {
                        int neworb = PadBoardAI7x6.getNewOrbRestrictChances(
                                mDropType, dropData);
                        if (neworb <= 5
                                && RandomUtil
                                        .getLuck(mPlusOrbCount[neworb] * 20)) {
                            neworb += 10;
                        }
                        if(lockPercent>0) {
                        	if(targetOrb == -1 && RandomUtil.getLuck(lockPercent)) {
    ...
    private void triggerAttack() {
        ...
        if (mFiredSkill == null || mFiredSkill.getType() != SkillType.ST_CLEAR_BOARD) {
            changeState(PadGameScene.GameState.GAME_ENEMY_TURN);
        }
        mEnvironment.triggerAttackPhase();
    }

    public boolean isClearBoard() {
        return mFiredSkill != null && mFiredSkill.getType() == SkillType.ST_CLEAR_BOARD;
    }
    ...
    private void loadSharedPreference() {
        ...
        mDropTime = Constants.DEFAULT_SECOND;//(int) (1000 * sp.getTimeLimit());
        mDropType = PadBoardAI7x6.getType(ComboMasterApplication.getsInstance().getDropOrbs());
    }
    ...
    public boolean skillFired(ActiveSkill skill, ICastCallback callback, boolean byEnemy) {
        if(!byEnemy) {
            mFiredSkill = skill;
        }
        ...
        case ST_TARGET_RANDOM:
            if (mCurrentState != PadGameScene.GameState.GAME_END) {
...
//                mStack.push(gameBoard);
        ...
        case ST_ADD_PLUS: {
            ...
            setChangeColorBoard(PadBoardAI7x6
                    .colorChangedBoard(gameBoard, addPlus), callback, false);
            return true;
        }
...
//				mStack.push(gameBoard);
				...
				setChangeColorBoard(PadBoardAI7x6
						.colorChangedBoard(board, addPlus), callback, byEnemy);
			}
            return true;
        ...
        case ST_LINE_RANDOM_CHANGE: {
        	...
        	int[][] board = PadBoardAI7x6.copy_board(gameBoard);
        	for(int i=1; i<=lineCount; ++i) {
        		int line = data.get(colorCount+1+i);
        		if(line<10) {
        			for(int x=0; x<PadBoardAI7x6.ROWS; ++x) {
        				int rand = RandomUtil.getInt(colorCount);
        				board[x][line] = colors.get(rand);
        			}
        		} else {
        			int pos = line%10;
        			for(int y=0; y<PadBoardAI7x6.COLS; ++y) {
        				int rand = RandomUtil.getInt(colorCount);
        ...
        case ST_COLOR_CHANGE: {
            ...
            //setChangeColorBoard(PadBoardAI7x6.colorChangedBoard(gameBoard, data), callback);
            setChangeColorBoard(PadBoardAI7x6.colorChangedBoard(gameBoard, data), data, callback, byEnemy);
            return true;
        }
            ...
            if (listSize >= 0) {
                ...
                for(int i=0; i<csize; ++i) {
                	...
                	for(int j=0; j<ccsize; ++j) {
                		...
                		int addition = (gameBoard[rc.row][rc.col] >= 10 && gameBoard[rc.row][rc.col] != PadBoardAI7x6.X_ORBS) ? 10: 0;
                		int cv = color;
                		if(cv<=5) {
                			cv += addition;
                		}
...
//    final int[] texture = {
//    		SimulateAssets.S_DARK_ID,SimulateAssets.S_FIRE_ID,SimulateAssets.S_LIGHT_ID,
//    		SimulateAssets.S_HEART_ID, SimulateAssets.S_WATER_ID,SimulateAssets.S_WOOD_ID
//    };


    public void clearBoard(final ICastCallback callback) {
        changeState(PadGameScene.GameState.GAME_ANIMATION);

        int[][] newBoard = PadBoardAI7x6.generateBoard(mDropType, false); //PadBoardAI7x6.getRandomBoard();//

        ArrayList<Pair<RowCol, RowCol>> dropList = new ArrayList<Pair<RowCol, RowCol>>();
        for(int row=0; row<PadBoardAI7x6.ROWS; ++row) {
            for (int k = PadBoardAI7x6.COLS-1; k >= 0; --k) {
                ...
                dropList.add(new Pair<RowCol, RowCol>(RowCol.make(
                        row, k), RowCol.make(row, k-PadBoardAI7x6.COLS)));
            }
        }
        for (Pair<RowCol, RowCol> pair : dropList) {
            final RowCol target = pair.first;
...
//                mgr.getFontSmall(), mgr.getTextureRegion(SimulateAssets.class,ATTR[attr]), vbom);
...
//        mAttrSprite[index] = ts;
    ...
    private void changeTheWorld(int time) {
        ...
        changeState(PadGameScene.GameState.GAME_PLAYER_TURN);
    ...
    private void initMonsterData(boolean nullAwoken) {
        ...
        if (mDropTime != 0) {
        	...
        	if(!nullAwoken) {
	            for (int i = 0; i < 6; ++i) {
	                ...
	                if (info != null) {
                        ...
                        poisonDropCount += info.getTargetAwokenCount(AwokenSkill.BLESSING_OF_POISON_DROP, false);
                        jammerDropCount += info.getTargetAwokenCount(AwokenSkill.BLESSING_OF_JAMMER_DROP, false);
	                }
	            }
        	}
    		...
    		if(singleUp == SinglePowerUp.MOVE) {
    			mDropTime += 1000;
    		} else if(singleUp == SinglePowerUp.MOVE_UP) {
    			mDropTime += 2000;
    		} else if(singleUp == SinglePowerUp.NO_DROP) {
    			normalDrop = false;
    		}
        ...
        if(!nullAwoken) {
	        for (int i = 0; i < 6; ++i) {
	            ...
	            if (info != null) {
	                for (int j = 0; j < 6; ++j) {
	                    mPlusOrbCount[j] += info
	                            .getTargetAwokenCount(awokenPlus[j], false);
	                }
	            }
	        }
        }
    ...
    public void createScene() {
        ...
        mBackground = new Sprite(0, 0, res.getTextureRegion("background.png"), vbom);
        mBackground.setScaleCenter(0f, 0f);
        mBackground.setScale(GameActivity.SCREEN_WIDTH/mBackground.getWidth());
        ...
        for(int i=1; i<=10; ++i) {
            ...
            text.setZIndex(6);
            mComboText.add(text);
            Text factor = new Text(0, 0, res.getFontStroke(), String.format("x10000.00 "), vbom);
            factor.setColor(Color.YELLOW);
            factor.setZIndex(7);
        ...
        mCrossShield = new Sprite(0, 0, res.getTextureRegion("cross_shield.png"), vbom);
        mCrossShield.setPosition(3, Constants.OFFSET_Y - 34);
        mCrossShield.setVisible(false);
        mCrossShield.setZIndex(3);
        mCrossShield.registerEntityModifier(new LoopEntityModifier(
        		new SequenceEntityModifier(
        				new AlphaModifier(Constants.SECOND_REFRESH, 1.0f, 0.0f), new AlphaModifier(Constants.SECOND_REFRESH, 0.0f, 1.0f)
        				)
        		));
		...
		for(int i=0; i<orbList.length; ++i) {
            ...
            mCache[i].setScale(scale*1.2f, scale*1.2f);
        ...
        for (int i = 0; i < PadBoardAI7x6.ROWS; ++i) {
            index = !index;
            for (int j = 0; j < PadBoardAI7x6.COLS; ++j) {
                int x = SimulatorConstants.ORB_SIZE_7x6 * i;
                int y = SimulatorConstants.ORB_SIZE_7x6 * j;
                boardSprite[i][j] = new Sprite(0, 0,
                        textureBoard[index ? 0 : 1], vbom);
                boardSprite[i][j].setScale(scale, scale);
                boardSprite[i][j].setPosition(x + SimulatorConstants.OFFSET_X, y
        ...
        mProgressbar = new Rectangle(SimulatorConstants.OFFSET_X,
                SimulatorConstants.OFFSET_Y - 15 - 8, PROGRESS_BAR_WIDTH, 15, vbom);
        ...
        for (int i = 0; i < 6; ++i) {
            Sprite s = new Sprite(0f, 0f, res.getTextureRegion(SimulateAssets.class, SimulateAssets.BIND_ID),
                    vbom);
        ...
        mEnvironment = new PadEnvironment(this);
    ...
    private boolean isSkipState() {
        return mCurrentState != PadGameScene.GameState.GAME_PLAYER_TURN
                || mChangeTheWorld.get();
    }
    ...
    private void init() {
        ...
        int[][] cache = null;//instance.getBoardCache();
        if (cache == null) {
            gameBoard = PadBoardAI7x6.generateBoardWithPlus(mDropType, false,
                    mPlusOrbCount);
        } else {
            gameBoard = PadBoardAI7x6.copy_board(cache);
        }
...
//        mReplay = new GameReplay();
        ...
        changeState(PadGameScene.GameState.GAME_INIT);
    }
...
//        mHpText.setVisible(!display);
    ...
    private boolean onGameRun(Scene pScene, TouchEvent pSceneTouchEvent) {
        if (mCurrentState == PadGameScene.GameState.GAME_ANIMATION) {
        ...
        int indexX = (int) x / SimulatorConstants.ORB_SIZE_7x6;
        int indexY = (int) y / SimulatorConstants.ORB_SIZE_7x6;
        ...
        if (pSceneTouchEvent.getAction() == TouchEvent.ACTION_DOWN) {
            if (PadBoardAI7x6.isValid(indexX, indexY)) {
                ...
                orbInHand.setPosition(x - 48 + SimulatorConstants.OFFSET_X, y - 48
                        + SimulatorConstants.OFFSET_Y);
            ...
            if (PadBoardAI7x6.isValid(currentX, currentY)
                ...
                // if position more than 2, skip it...
				 if (Math.abs(indexX - currentX) >= 2 || Math.abs(indexY - currentY) >= 2) {
	                	...
	                	if(PadBoardAI7x6.isValid(oriX, oriY)) {
	                		indexX = oriX;
	                		indexY = oriY;
	                	} else if(PadBoardAI7x6.isValid(oriX, currentY)) {
	                		indexX = oriX;
	                	} else if(PadBoardAI7x6.isValid(currentX, oriY)) {
	                		indexY = oriY;
	                	} else {
                ...
                if (!mCountDown.get()) {
                    mCountDown.set(true);
                    if(mTimeFixed == 0) {
                        mTimeRemain = mDropTime + mAdjustTime;
                        if (mAdjustTimeX != 0) {
                            mTimeRemain = (int) (mTimeRemain * mAdjustTimeX / 1000f);
                        }
                    } else {
                        mTimeRemain = mTimeFixed;
                    }
...
//                    mStack.push(gameBoard);
    ...
    private MatchBoard7x6 hasBomb() {
        MatchBoard7x6 matchBoard = PadBoardAI7x6.findMatches(gameBoard, removeN, cantRemove);
        for(int i = 0; i< PadBoardAI7x6.ROWS; ++i) {
            for(int j=0; j<PadBoardAI7x6.COLS; ++j) {
    ...
    private void bombExplode(MatchBoard7x6 mb) {
        int[][] board = PadBoardAI7x6.create_empty_board();
        int bombCount = 0;
        for(int i=0; i<PadBoardAI7x6.ROWS; ++i) {
            for(int j=0; j<PadBoardAI7x6.COLS; ++j) {
                if (gameBoard[i][j] != PadBoardAI.X_ORBS && (gameBoard[i][j]%10) == 9 &&
        ...
        if(mb != null) {
            ...
            handler.post(new Runnable() {
                ...
                public void run() {
                    ...
                    final MatchBoard7x6 matchBoard = PadBoardAI7x6.findMatches(gameBoard, removeN, cantRemove);
    ...
    private boolean unlockAction(List<Match7x6> matches) {
        ...
        for(int i=0; i<6; ++i) {
            ...
            if(!isNullAwoken() && info != null &&
                    info.getTargetAwokenCount(AwokenSkill.L_ATTACK, false)>0) {
    ...
    private float HALF_ORB_SIZE_PLUS_1414 = SimulatorConstants.HALF_ORB_SIZE_7x6 * 1.41421356237f;

    private void swapOrb(int nx, int ny, int cx, int cy) {
        final float toX = cx * SimulatorConstants.ORB_SIZE_7x6 + SimulatorConstants.OFFSET_X;
        final float toY = cy * SimulatorConstants.ORB_SIZE_7x6 + SimulatorConstants.OFFSET_Y;
        float fromX = nx * SimulatorConstants.ORB_SIZE_7x6 + SimulatorConstants.OFFSET_X;
        float fromY = ny * SimulatorConstants.ORB_SIZE_7x6 + SimulatorConstants.OFFSET_Y;
        ...
        int angle = 0;
        boolean isAdd = true;
        float radius = Constants.HALF_ORB_SIZE_7x6;
        ...
        if (isBlack(ns.getColor()) && ns.getChildByTag(TAG_SUPER_DARK) == null) {
            ns.setColor(Color.WHITE);
        }
        if (isBlack(s.getColor()) && s.getChildByTag(TAG_SUPER_DARK) == null) {
    ...
    private synchronized void changeState(PadGameScene.GameState state) {
        LogUtil.d("previous=", mCurrentState, " , next=", state);
        if (mCurrentState != state) {
            mCurrentState = state;

            if (mCurrentState == PadGameScene.GameState.GAME_PLAYER_TURN) {
                mFiredSkill = null;
    ...
    private void setBoardAlpha(float alpha, float to, final ICastCallback callback) {
    ...
    private Sprite getSprite(int orb, Sprite olds) {
        ...
        if (olds != null) {
            if(isBlack(olds.getColor())) {
                ...
                IEntity entity = olds.getChildByTag(TAG_SUPER_DARK);
                if(entity != null) {
                    olds.detachChild(TAG_SUPER_DARK);
    ...
    private void setCloud(final List<Integer> data) {
        ...
        if(data.get(1) == -1) {
            ...
            if (line >= 10) {
//				for (int i = 0; i < x; ++i) {
                int x = (line == 10)? 0:PadBoardAI7x6.ROWS-1;
                for (int j = 0; j < PadBoardAI7x6.COLS; ++j) {
                    int posx = SimulatorConstants.ORB_SIZE_7x6 * (x + offset) + SimulatorConstants.OFFSET_X - 6;
                    int posy = SimulatorConstants.ORB_SIZE_7x6 * (j) + SimulatorConstants.OFFSET_Y - 6;
...
//				}
                ...
                int y = (line == 0)? 0:PadBoardAI7x6.COLS-1;
                for (int i = 0; i < PadBoardAI7x6.ROWS; ++i) {
//					for (int j = 0; j < y; ++j) {
                    int posx = SimulatorConstants.ORB_SIZE_7x6 * (i) + SimulatorConstants.OFFSET_X - 6;
                    int posy = SimulatorConstants.ORB_SIZE_7x6 * (y+offset) + SimulatorConstants.OFFSET_Y - 6;
                    Sprite s = new Sprite(0, 0, res.getTextureRegion("cloud.png"), vbom);
                    s.setScale(scale, scale);
                    s.setPosition(posx, posy);
                    s.setZIndex(4);
...
//					}
            ...
            int offset_x = RandomUtil.getInt(PadBoardAI7x6.ROWS - x + 1);
            int offset_y = RandomUtil.getInt(PadBoardAI7x6.COLS - y + 1);

            float scale = getScaleRatio();
            for (int i = 0; i < x; ++i) {
                for (int j = 0; j < y; ++j) {
                    int posx = SimulatorConstants.ORB_SIZE_7x6 * (i + offset_x) + SimulatorConstants.OFFSET_X - 6;
                    int posy = SimulatorConstants.ORB_SIZE_7x6 * (j + offset_y) + SimulatorConstants.OFFSET_Y - 6;
                    Sprite s = new Sprite(0, 0, res.getTextureRegion("cloud.png"), vbom);
                    s.setScale(scale, scale);
                    s.setPosition(posx, posy);
                    s.setZIndex(4);
                ...
                if ( isTarget ) {
                    ...
                    if (data.get(size) == -1) { // all
                        ...
                        for (int i = 1; i < size; i += 2) {
                            ...
                            int offset = data.get(i+1);
                            if (line == 0) { // top
                                for(int j=0; j<PadBoardAI7x6.ROWS; ++j) {
                                    Sprite s = sprites[j][offset];
                                    setSuperDarkOrb(res, s, turns);
                                }
                            } else if (line == 1) { // bottom
                                for(int j=0; j<PadBoardAI7x6.ROWS; ++j) {
                                    Sprite s = sprites[j][PadBoardAI7x6.COLS-offset-1];
                                    setSuperDarkOrb(res, s, turns);
                                }
                            } else if (line == 2) { // left
                                for(int j=0; j<PadBoardAI7x6.COLS; ++j) {
                                ...
                                for(int j=0; j<PadBoardAI7x6.COLS; ++j) {
                                    Sprite s = sprites[PadBoardAI7x6.ROWS-offset-1][j];
                                    setSuperDarkOrb(res, s, turns);
                                }
                            }
                        }
                        ...
                        if(mEnvironment.resistBadEffect(AwokenSkill.RESISTANCE_DARK)) {
                            if ( isTarget ) {
                                ...
                                for (int i = 1; i < size; i += 2) {
                                    ...
                                    sprites[x][y].detachChild(TAG_SUPER_DARK);
                                }
                            } else {
                                for(int i=0; i<PadBoardAI7x6.ROWS; ++i) {
                                    for (int j=0; j<PadBoardAI7x6.COLS; ++j) {
    ...
    private void setChangeLineBoard(ActiveSkill skill, final ICastCallback callback, final boolean byEnemy) {
        changeState(PadGameScene.GameState.GAME_ANIMATION);
        resetCombo();
        ...
        engine.runOnUpdateThread(new Runnable() {
            ...
            public void run() {
            	...
            	if(byEnemy) {
            		counterSpell[0] = mEnvironment.resistBadEffect(AwokenSkill.RESISTANCE_POISON);
            		counterSpell[1] = mEnvironment.resistBadEffect(AwokenSkill.RESISTANCE_JAMMER);
            	}
                ...
                for(int index=0; index<loop; index+=2) {
                    ...
                    if (pos < 3 || pos == 5 || pos == 6) {
                        ...
                        int y = SimulatorConstants.ORB_SIZE_7x6 * line;
                        for (int i = 0; i < PadBoardAI7x6.ROWS; ++i) {
                            ...
                            int x = SimulatorConstants.ORB_SIZE_7x6 * i;
                            int addition = 0;
                            if(gameBoard[i][line] >= 10 && gameBoard[i][line] != PadBoardAI7x6.X_ORBS
                            		&& toOrb<=5) {
                            	addition = 10;
                            }
                        	...
                        	if( (counterSpell[0] && isPoison(toOrb)) ||
                        		...
                        		ns.registerEntityModifier(new ScaleModifier(SimulatorConstants.SECOND_REFRESH, 0.4f, 2.0f){
                        			protected void onModifierFinished(IEntity pItem) {
                        				pItem.registerEntityModifier(new ScaleModifier(SimulatorConstants.SECOND_REFRESH, 2.0f, 0.1f){
                        					protected void onModifierFinished(IEntity pItem) {
                        						pItem.setVisible(false);
                        						addToDetachList(pItem);
                        					};
                        				...
                        				s.registerEntityModifier(new ScaleModifier(SimulatorConstants.SECOND_REFRESH, 2.4f, getScaleRatio()));
                        			};
                        		});
                        	} else {
                        ...
                        int x = SimulatorConstants.ORB_SIZE_7x6 * line;
                        for (int j = 0; j < PadBoardAI7x6.COLS; ++j) {
                            ...
                            int y = SimulatorConstants.ORB_SIZE_7x6 * j;
                            int addition = (toOrb<=5 && gameBoard[line][j] >= 10 && gameBoard[line][j] != PadBoardAI7x6.X_ORBS) ? 10
                                    : 0;
        ...
        registerUpdateHandler(new TimerHandler(SimulatorConstants.SECOND_REFRESH + 0.1f,
                new ITimerCallback() {
    ...
    public void setChangeColorBoard(final int[][] board, final List<Integer> changeList, final ICastCallback callback,
            ...
            public void run() {
                ...
                for (int i = 0; i < PadBoardAI7x6.ROWS; ++i) {
                    for (int j = 0; j < PadBoardAI7x6.COLS; ++j) {
                        ...
                        if (gameBoard[i][j] != board[i][j] || changeSet.contains(gameBoard[i][j]%10)) {
                        	if(gameBoard[i][j]>=20 && (gameBoard[i][j]+10) != toOrb) {
                        	...
                        	if( (counterSpell[0] && isPoison(toOrb)) ||
                        			(counterSpell[1] && isJarma(toOrb))) {
                        		ns.registerEntityModifier(new ScaleModifier(SimulatorConstants.SECOND_REFRESH, 0.4f, 1.6f){
                        			protected void onModifierFinished(IEntity pItem) {
                        				pItem.registerEntityModifier(new ScaleModifier(SimulatorConstants.SECOND_REFRESH, 1.0f, 0.1f){
                        					protected void onModifierFinished(IEntity pItem) {
                        						pItem.setVisible(false);
                        						addToDetachList(pItem);
                        					};
    ...
    public void setChangeColorBoard(final int[][] board, final ICastCallback callback, final boolean byEnemy) {
        changeState(PadGameScene.GameState.GAME_ANIMATION);
...
//                            attachChild(s);
    ...
    private void setGameBoard(final int[][] board, final IAnimationCallback callback, final ICastCallback castCallback, final boolean byEnemy) {
        changeState(PadGameScene.GameState.GAME_ANIMATION);
        //PadBoardAI7x6.copy_board(board, gameBoard);
    ...
    public void setGameBoard(int[][] board, ICastCallback callback, boolean byEnemy) {
        setGameBoard(board, null, callback, byEnemy);
    }

    @Override
    public String getSceneName() {
        return PadGameScene7x6.class.getSimpleName();
    }
    ...
    public void onFireSkills(boolean start) {
        if (start) {
            changeState(PadGameScene.GameState.GAME_FIRE_SKILL);
        } else {
            changeState(PadGameScene.GameState.GAME_PLAYER_TURN);
        }
    }
    			...
    			Observable.from(mEnvironment.getEnemies())
    				...
    				.subscribe(new Action1<Enemy>() {
						...
						public void call(Enemy enemy) {
							if(!enemy.dead()) {
								AttackValue attack = new AttackValue(5000000, 0, 0);
								enemy.dealtDamageDirect(attack, 1.0f, new AnimationCallback() {
    ...
    @Override
    public PadGameScene.GameState getState() {
    ...
    public Pair<Integer, Integer> getSize() {
    	return new Pair<Integer, Integer>(PadBoardAI7x6.ROWS, PadBoardAI7x6.COLS);
    }
                    ...
                    for(int i=0; i<PadBoardAI7x6.ROWS; ++i) {
                        for (int j = 0; j < PadBoardAI7x6.COLS; ++j) {
                            ...
                            if(isBlack(s.getColor())) {
                                IEntity dark = s.getChildByTag(TAG_SUPER_DARK);
                                if(dark != null) {
                                    Text text = (Text) dark.getChildByTag(TAG_SUPER_DARK_TEXT);
