...
import com.a30corner.combomaster.engine.sprite.TextSprite;
...
public class CalculatorGameScene extends BaseMenuScene implements
    ...
    private TextSprite[] mAttrSprite = new TextSprite[6];
...
//    private IMenuItem mHelpMenu;
    ...
    private void doRemoval(final MatchBoard board, final int current) {
            ...
            if (comboCount <= mComboList.size()) {
                ...
                text.setZIndex(6);
                mComboList.add(text);
                factor = new Text(0f, 0f, ResourceManager.getInstance().getFontStroke(), "x1000000.00 ", vbom);
            ...
            for (int i = 0; i < PadBoardAI.ROWS; ++i) {
                ...
                if (target != null && orb != null) {
                    ...
                    if (mDropRateSkill != null) {
                        ...
                        int size = data.size();
                        dropData = new ArrayList<Pair<Integer, Integer>>(
                                (size - 1) / 2);
    ...
    private void showScore(boolean fromMenu) {
        if ((fromMenu || mShowDialog) && mCurrentState != GameState.GAME_REPLAY) {
            ...
            final List<Pair<Double, Double>> data = DamageCalculator.calculate(
                    mTeam, mScoreBoard, active, 0, mSettings);
            final double recovery = DamageCalculator.calculateRecovery(mTeam,
                    mScoreBoard, active, mSettings);
            final double poison = DamageCalculator.calculatePoison(mTeam,
                    mScoreBoard, active);
    ...
    private void loadSharedPreference() {
        ...
        mDropType = PadBoardAI.getType(sp.getDropsInCalcMode());
    ...
    private void skillFired(Sprite self, ActiveSkill skill) {
        ...
        case ST_CHANGE_SELF_ATTR: {
            ...
            if (index != -1) {
                createChangeSelfAttrSprite(index, skill);
            }
            break;
        }
        ...
        case ST_COLOR_CHANGE:
            if (mCurrentState != GameState.GAME_END) {
                ...
                setChangeColorBoard(PadBoardAI.colorChangedBoard(board, data));
            }
            break;
    ...
    private void createTimeExtendSprite(ActiveSkill skill) {
        ...
        mTimeExtendSprite = new Sprite(0f, 0f, pTextureRegion,
                mResMgr.getVBOM());
    ...
    private void createDropSkillSprite(ActiveSkill skill) {
        ...
        for (int i = 1; i < size; i += 2, ++index) {
            ...
            mDropRateSprite[index] = new Sprite(0f, 0f, pTextureRegion,
                    mResMgr.getVBOM());
            mDropRateSprite[index].setPosition(
                    x + index * pTextureRegion.getWidth() + 8, y
                            - pTextureRegion.getHeight() + 8);
            attach(mDropRateSprite[index]);
        }
        mDropRateTimes = data.get(0);
        mDropRateText = new Text(0f, 0f, mResMgr.getFontSmall(),
                String.valueOf(mDropRateTimes), mResMgr.getVBOM());
        mDropRateText.setPosition(mDropRateSprite[index - 1].getX()
                + mDropRateSprite[index - 1].getWidth() + 8,
                mDropRateSprite[index - 1].getY());
        ...
        for (int i = 0; i < 6; ++i) {
            ...
            if (info != null && !mBindStatus[i]) {
    			for(Integer awk : awoken) {
                    total += info
                            .getTargetAwokenCount(AwokenSkill.get(awk), ComboMasterApplication.getsInstance().isCopMode());
    			}
            }
        }
    ...
    private void skillFired(Skill skill) {
        ...
        case SKILL_STANCE:
            if (mCurrentState != GameState.GAME_END) {
                ...
                int[][] board = new int[PadBoardAI.ROWS][PadBoardAI.COLS];
                PadBoardAI.copy_board(gameBoard, board);
                // PadBoardAI.copy_board(board, previousBoard);
                // saveCurrentBoard();
                int len = data.size();
    ...
    private void initMonsterData() {
        boolean isCopMode = ComboMasterApplication.getsInstance().isCopMode();
        if (!mNullAwoken) {
            if (mDropTime != 0) {
                for (int i = 0; i < 6; ++i) {
                    MonsterInfo info = mTeam.getMember(i);
                ...
                if (info != null) {
                    for (int j = 0; j < 6; ++j) {
                        mPlusOrbCount[j] += info
                                .getTargetAwokenCount(awokenPlus[j], isCopMode);
                    }
                }
            }
        }
    ...
    protected void onManagedUpdate(float pSecondsElapsed) {
        List<IEntity> removed = null;
        synchronized (mRemoveList) {
            int size = mRemoveList.size();
            if (size >= RELEASE_THRESHOLD) {
                removed = new ArrayList<IEntity>(mRemoveList);
                mRemoveList.clear();
            }
        }
        if(removed != null) {
    ...
    private void loadSpecialMode() {
        mLvText = new Text[6];
        IFont font = mResMgr.getFontStroke();
        ...
        mTeam.init(Constants.MODE_SPECIAL, mStagePowerUp, ComboMasterApplication.getsInstance().isCopMode());

        mLvMenu = new ColorMenuItemDecorator(new SpriteMenuItem(MENU_SETLV,
                mResMgr.getTextureRegion(GameUIAssets.class, GameUIAssets.IC_LV_ID), vbom), new Color(.5f,
                .5f, .5f), Color.WHITE);
        mLvMenu.setScale(0.8f, 0.8f);
        addMenuItem(mLvMenu);
        mLvMenu.setPosition(mStepCountText.getX() + mLvMenu.getWidth() * 2,
    ...
    public void createScene() {
        ...
        for(int i=1; i<=10; ++i) {
            ...
            text.setZIndex(6);
            mComboList.add(text);
            Text factor = new Text(0, 0, res.getFontStroke(), String.format("x1000000.00 "), vbom);
            factor.setColor(Color.YELLOW);
            factor.setZIndex(7);
        ...
        mSwapSprite.setPosition(Constants.OFFSET_X,
                mProgressbar.getY() - 8 - 88);
        ...
        settings.setPosition(result.getX() + result.getWidth() + 16,
                startY + 16);
        ...
        // special menu ...
//        mHelpMenu = new ColorMenuItemDecorator(new SpriteMenuItem(MENU_HELP,
//                res.getTextureRegion(GameUIAssets.class, GameUIAssets.IC_HELP_ID), vbom), new Color(.5f, .5f,
//                .5f), Color.WHITE);
//        addMenuItem(mHelpMenu);
//        mHelpMenu.setPosition(mStepCountText.getX() + mHelpMenu.getWidth() * 2,
//                mStepCountText.getY() - 10);
        ...
        mShowPathText
                .setPosition(mCheckBox[0].getX() + 32, mCheckBox[0].getY());
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            mSkillCheckBox[i].setPosition(Constants.OFFSET_X + 32, h
                    + checkBoxHeight + 12);
        ...
        mSkillFiredText.setPosition(mCheckBox[0].getX() + 32,
                mSkillCheckBox[0].getY());
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            mDropCheckBox[i].setPosition(Constants.OFFSET_X + 32, h
                    + (checkBoxHeight + 12) * 2);
        ...
        mContinueDropText.setPosition(mCheckBox[0].getX() + 32,
                mDropCheckBox[0].getY());
        ...
        // load special mode ( Lv ) , need run after character sprite and
        // ic_help ready
    ...
    private void initEditSprites(float menuHeight) {
        float offsetX = (GameActivity.SCREEN_WIDTH - 6 * textureOrbs.get(0)
                .getWidth()) / 2f;
        float offsetY = menuHeight * 1.65f + 15;
        for (int i = 0; i < 2; ++i) {
            for (int j = 0; j < 4; ++j) {
                final int pos = i * 4 + j;
                    ...
                    try {
                        ...
                        activity.runOnUiThread(new Runnable() {
                            ...
                            public void run() {
                                Toast.makeText(
                                        activity,
                                        activity.getString(R.string.str_need_album_app),
    ...
    public boolean onMenuItemClicked(MenuScene pMenuScene, IMenuItem pMenuItem,
        ...
        case MENU_HELP:
            activity.runOnUiThread(new Runnable() {
                ...
                @Override
                public void run() {
                    DialogUtil.getTinyUrlDialog(activity, gameBoard, mTeam,
                            mPowerUpSkill, mSettings).show();
                }
            });
                ...
                if (mSettingDialog == null) {
                    ...
                    mSettingDialog.setCallback(new ISettingsCallback() {
                        ...
                        public void onResult(Map<String, String> data) {
                            float seconds = Float.parseFloat(data
                                    .get("seconds"));
                            ...
                            if (old != mSwapIndex) {
                                ...
                                if (mSwapIndex != 0) {
                                    ...
                                    mSkill[0].setPosition(Constants.OFFSET_X
                                            + mSwapIndex * (88 + 1),
                                            mProgressbar.getY() - 8 - 88);
                                    mSkill[mSwapIndex].setPosition(
                                            Constants.OFFSET_X,
                                            mProgressbar.getY() - 8 - 88);
                                    if (old != 0) {
                                        mSkill[old].setPosition(
                                                Constants.OFFSET_X + old
                                                        * (88 + 1),
                                                mProgressbar.getY() - 8 - 88);
                                    }
                                } else {
                                    ...
                                    mSkill[old].setPosition(Constants.OFFSET_X
                                            + old * (88 + 1),
                                            mProgressbar.getY() - 8 - 88);
                                    mSkill[0].setPosition(Constants.OFFSET_X,
                                            mProgressbar.getY() - 8 - 88);
                            ...
                            if (data.containsKey("drop0")) {
                                final ActiveSkill skill = new ActiveSkill(
                                        SkillType.ST_DROP_RATE);
                                int droprate = Integer.parseInt(data
                                        .get("droprate"));
                                ...
                                if (data.containsKey("drop1")) {
                                    int type1 = Integer.parseInt(data
                                            .get("drop1"));
                            ...
                            final Boolean isHpChecked = Boolean
                                    .parseBoolean(data.get("isHpChecked"));
                            final Float hpPercent = Float.parseFloat(data
                                    .get("hp"));
                            engine.runOnUpdateThread(new Runnable() {
                                ...
                                public void run() {
                                    ...
                                    mHpText.setText(String.format("%.2f%%",
                                            percent));
                                }
                            });
...
//        mHelpMenu.setVisible(false);
    ...
    private void setGameBoard(int[][] board) {
        setGameBoard(board, null, false);
    }

    @Override
    public String getSceneName() {
        return CalculatorGameScene.class.getSimpleName();
    }
