...
public class ConditionUsedIf extends ConditionUsed {

	public ConditionUsedIf(int... data) {
		super(Type.USED_IF, data);
	}
	...
	public boolean checkCondition(IEnvironment env, Enemy enemy) {
		...
		for(int i=0; i<count; ++i) {
			...
			if(type == Type.HP.ordinal()) {
				...
				offset += 3;
			} else if (type == Type.ENEMY_DEBUFF.ordinal()) {
				check = new ConditionEnemyDebuff().checkCondition(env, enemy);
				offset += 1;
			...
			} else if(type == Type.TURN.ordinal()) {
				check = new ConditionTurns(data.get(offset+1)).checkCondition(env, enemy);
				offset+=2;
