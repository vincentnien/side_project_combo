...
public class UltimateArena5 extends IStage {
    ...
    public void create(IEnvironment env,
        ...
        data = new ArrayList<Enemy>();
        {
            ...
            for (int i = 0; i < 4; ++i) {
                ...
                int choose = ids[rnd];
                data.add(Enemy.create(env, choose, 6, 26000, 100000000, 1, getMonsterAttrs(env, choose), getMonsterTypes(env, choose)));
                zero = data.get(i);
...
//                }
        ...
        data = new ArrayList<Enemy>();
        {
            ...
            for (int i = 0; i < 1; ++i) {
                ...
                if (choose == 1920) {
                    ...
                    seq = new SequenceAttack();
                    seq.addAttack(new EnemyAttack().addAction(new LockSkill(text[3].title, text[3].description, 5)).addAction(new DropLockAttack(text[5].title, text[5].description, 10, 4, 100, 2, 100)).addPair(new ConditionAlways(), new Attack(140)));
                    zero.attackMode.addAttack(new ConditionFirstStrike(), seq);

                    seq = new SequenceAttack();
                    seq.addAttack(new EnemyAttack().addAction(new Attack(text[6].title, text[6].description,150)).addPair(new ConditionUsed(), new RandomChange( 4, 4)));
                    seq.addAttack(new EnemyAttack().addAction(new Attack(text[7].title, text[7].description,200)).addPair(new ConditionUsed(), new RandomChange( 4, 8)));
                    seq.addAttack(new EnemyAttack().addAction(new Attack(text[8].title, text[8].description,250)).addPair(new ConditionUsed(), new RandomChange( 4, 16)));
                    seq.addAttack(new EnemyAttack().addAction(new Attack(text[9].title, text[9].description,500)).addPair(new ConditionAlways(), new Transform(4)));
                    zero.attackMode.addAttack(new ConditionAlways(), seq);
        ...
        data = new ArrayList<Enemy>();
        {
            ...
            for (int i = 0; i < 1; ++i) {
                ...
                text = loadSubText(list, choose);
                if (choose == 2393) {
                    data.add(Enemy.create(env, 2393, 67180000, 28224, 1692, 1, getMonsterAttrs(env, 2393), getMonsterTypes(env, 2393)));
                    zero = data.get(i);
                    zero.addOwnAbility(BuffOnEnemySkill.Type.KONJO, 50);
        ...
        data = new ArrayList<Enemy>();
        {
            ...
            for (int i = 0; i < 1; ++i) {
                ...
                text = loadSubText(list, choose);
                if (choose == 3074) {
                    data.add(Enemy.create(env, 3074, 33600000, 9045, 400000, 1, getMonsterAttrs(env, 3074), getMonsterTypes(env, 3074)));
                    zero = data.get(i);
        ...
        data = new ArrayList<Enemy>();
        {
            ...
            for (int i = 0; i < 1; ++i) {
                ...
                if (choose == 4358) {
                    data.add(Enemy.create(env, 4358, 93600000, 18256, 1536, 2, getMonsterAttrs(env, 4358), getMonsterTypes(env, 4358)));
                    zero = data.get(i);
        ...
        data = new ArrayList<Enemy>();
        {
            ...
            for (int i = 0; i < 1; ++i) {
                ...
                if (choose == 3642) {
                    data.add(Enemy.create(env, 3642, 433333333, 39858, 6688, 4, getMonsterAttrs(env, 3642), getMonsterTypes(env, 3642)));
                    zero = data.get(i);
        ...
        data = new ArrayList<Enemy>();
        {
            ...
            for (int i = 0; i < 3; ++i) {
                ...
                int rnd = RandomUtil.getInt(ids.length);
                int choose = ids[rnd];
                data.add(Enemy.create(env, choose, 24, 7707, 12000000, 1, getMonsterAttrs(env, choose), getMonsterTypes(env, choose)));
                zero = data.get(i);
