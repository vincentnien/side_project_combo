...
import com.a30corner.combomaster.scene.MultiplePadGameScene;
import com.a30corner.combomaster.scene.MultiplePadGameScene7x6;
...
public class MultiplePadEnvironment implements IEnvironment {
    ...
    private PadContext padContext = new PadContext();
    
    // stage info
    private boolean nullAwoken = false;
    
    private String mStage;
    
    private int currentTeam = 0;
    private int currentStage = 1;
    private SparseArray<ArrayList<Enemy>> mStageEnemies = new SparseArray<ArrayList<Enemy>>(); 
    
    public MultipleTeam mTeamData;
    public Member[][] mMemberList = new Member[2][6];
    
    private Color[] ATTACK_COLOR = new Color[6];
    
    private Text mStageTop;
    private Text mStageText;
    private Text mJumpTo;
    private Text mPass;
    private HPEntity mHp;
    
    private boolean jumpUsed = false;

    private SkillEntity mLeaderSwitch = null;
    private Map<String, SkillEntity> mSkillList = new HashMap<String, SkillEntity>();
    private ActiveSkill mDarkSkill = null;
    
    private SpritePool mBulletPool;
    
    private Toast customToast;
    private int mStoneUsed;
    
    private List<Pair<Text, Text>> mDamageList = new ArrayList<Pair<Text, Text>>(6);
    private List<Pair<Text, Text>> mDamage2List = new ArrayList<Pair<Text, Text>>(6);
    //private List<Pair<NumberText, NumberText>> mDamageList = new ArrayList<Pair<NumberText, NumberText>>(6);
    
    private Pair<Float, Float> mCrossShield = null;
    ...
    public int getTargetAwokenCount(MonsterInfo info,AwokenSkill skill) {
        ...
        return info.getTargetAwokenCount(skill, isMultiMode());
    }
    ...
    class ExecutorHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            case MSG_ENEMY_FIRST_STRIKE: {
                ...
                actionList.get(subindex).doAction(MultiplePadEnvironment.this, getEnemies().get(index), new ICastCallback() {
            ...
            case MSG_POISON_DAMAGE: {
            	...
            	boolean hasLstAttack = matches.size()>0 &&
            			(PadEnvironment.isAdditionAttack(getMember(0).info) || PadEnvironment.isAdditionAttack(getMember(5).info)
            ...
            case MSG_END_OF_ATTACK:
                ...
                getScene().engine.runOnUpdateThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        for(int i=0; i<6; ++i) {
                        	if(mMemberList[currentTeam][i] != null) {
                        		mMemberList[currentTeam][i].sprite.setAlpha(1f);
                        	}
                        }
                        
                        for(Pair<Text, Text> pair : getDamageList()) {
                ...
                try {
                    ...
                    if(mLeaderSwitch != null && mLeaderSwitch.which == currentTeam && mLeaderSwitch.countDown()) {
            ...
            case MSG_START_ATTACK: 
            {
                ...
                for(int i=0; i<6; ++i) {
                	...
                	MonsterInfo info = mMemberList[currentTeam][i].info;
                	if (info == null) {
                		continue;
                	}
                	if(!awokenLocked && !mMemberList[currentTeam][i].isBinded()) {
                ...
                long delay = 0L;
                if(hasMatches && mTeamData.getCurrentHp()<mTeamData.getHp()) {
                	delay = 500L;
                }
            ...
            case MSG_CHECK_HP: {
                ...
                if (hasMatches && mTeamData.getCurrentHp()<mTeamData.getHp()) {
	                ...
	                if(!awokenLocked) {
		                for(int i=0; i<6; ++i) {
		                	if(mMemberList[currentTeam][i] == null || 
		                			mMemberList[currentTeam][i].isBinded()) {
		                		continue;
		                	}
		                	count += getTargetAwokenCount(mMemberList[currentTeam][i].info, AwokenSkill.AUTO_RECOVER);
		                }
	                }
	                if(count>0) {
	                ...
	                Member leader = mMemberList[currentTeam][0];
	                Member friend = mMemberList[currentTeam][5];
                ...
                List<Pair<Text, Text>> damageList = getDamageList();
                for(int i=0; i<6; ++i) {
                	if(mMemberList[currentTeam][i] == null) {
                		continue;
                	}
                	mMemberList[currentTeam][i].sprite.setAlpha(.5f);
                    Pair<AttackValue, AttackValue> attacks = data.get(i);
                    Pair<Text, Text> damageText = damageList.get(i);
            ...
            case MSG_PLAYER_ATTACK:
                ...
                List<Enemy> target = AttackModule.findSuitableEnemies(mTeamData.getMember(index),mStageEnemies.get(currentStage), attack, true);
                if (target.size() == 0) {
                    sendEmptyMessage(MSG_END_OF_ATTACK);
                    return ;
                }
        ...
        private void fireAdditionAttack(final AttackValue attack, final List<Enemy> target) {
            ...
            if(attack != null && attack.prop>=0 && attack.prop < 6) {
    ...
    public MultiplePadEnvironment(PlaygroundGameScene scene) {
...
//        calculateCrossShield();
    ...
    private void initMemberSprite() {
        ...
        for(int j=0; j<2; ++j) {
        	int offset = j * GameActivity.SCREEN_WIDTH;
	        for(int i=0; i<5; ++i) {
	            MonsterInfo info = mTeamData.getMember(j, i);
	            if (info != null) {
	                Member member = null;
	                ...
	                mMemberList[j][i] = member;
	            }
	        }
        }
        mMemberList[0][5] = mMemberList[1][0];
        mMemberList[1][5] = mMemberList[0][0];
        
        mMemberList[0][5].setPosition(Constants.OFFSET_X + 5 * (88 + 1),
	                            Constants.OFFSET_Y - 15 - 8 - 8 - 88);
        
        PlaygroundGameScene scene = getScene();
        for(int j=0; j<2; ++j) {
	        for(int i=0; i<5; ++i) {
	            Member member = mMemberList[j][i];
	            if (member != null) {
    ...
    private void initStageSprite() {
        ...
        mJumpTo = new Text(0, 0, res.getFont(), "JUMP", vbom) {
        	...
        	public boolean onAreaTouched(TouchEvent pSceneTouchEvent,
        		...
        		if(jumping == false && getScene().getState() == GameState.GAME_PLAYER_TURN) {
        			getScene().getActivity().runOnUiThread(new Runnable() {
        ...
        mPass = new Text(0, 0, res.getFont(), "Pass", vbom) {
        	@Override
        	public boolean onAreaTouched(TouchEvent pSceneTouchEvent,
        			float pTouchAreaLocalX, float pTouchAreaLocalY) {
        		if(changing == false && getScene().getState() == GameState.GAME_PLAYER_TURN) {
        			changePlayer(getScene());
        		}
        		return true;
        	}
        };
        getScene().registerTouchArea(mPass);
        ...
        mPass.setPosition(8, 84f);
        getScene().attach(Arrays.asList(mJumpTo, mStageTop, mPass));
        mStageText = new Text(0f, 0f, res.getFont(),
                "Stage 01/01", vbom);
        mStageText.setBlendFunction(GL10.GL_SRC_ALPHA, GL10.GL_ONE_MINUS_SRC_ALPHA);

        float y = mMemberList[0][0].sprite.getY();
        ...
        for(int j=0; j<2; ++j) {
	        for(int i=0; i<6; ++i) {
	            ...
	            if(j==0) {
	            	...
	            	mDamage2List.add(new Pair<Text, Text>(text1, text2));
	            }
	        }
        }
        
        mHp = new HPEntity(this);
        mHp.init(mTeamData.getHp());
        mHp.attachScene();
        //mHp.attach();
        
    }
    
    private List<Pair<Text, Text>> getDamageList() {
    	return currentTeam==0? mDamageList:mDamage2List;
    }
    ...
    public void bindRecover(final int recover) {
    	...
    	for(int i=0; i<2; ++i) {
    		for(int j=0; j<5; ++j) {
    			Member member = mMemberList[i][j];
    			if(member != null && member.info != null && member.isBinded()) {
    				member.bindRecover(recover);
    				if(!member.isBinded()) {
    					mTeamData.bindPet(i, j, false);
    					changed = true;
    				}
    			}
    		}
        ...
        getScene().clearBoard(new ICastCallback() {
			...
			public void onCastFinish(boolean casted) {
		        try {
		            ...
		            for(int j=0; j<2; ++j) {
			            for(int i=0; i<5; ++i) {
			            	if(mMemberList[j][i] != null) {
			            		skillBoost += getTargetAwokenCount(mMemberList[j][i].info, AwokenSkill.SKILL_BOOST);
                                skillBoost += getTargetAwokenCount(mMemberList[j][i].info, AwokenSkill.SKILL_BOOST_PLUS)*2;
			            	}
			            }
		            }
		            ...
		            for(int j=0; j<2; ++j) {
			            for(int i=0; i<5; ++i) {
			            	if(mMemberList[j][i] != null) {
			            		mMemberList[j][i].setCd(skillBoost);
			            	}
			            }
		            }
    ...
    private void displayNextTeam(final PlaygroundGameScene scene) {
        currentTeam = (currentTeam==0)? 1:0;
        mTeamData.setCurrentTeam(currentTeam);
        int offset = currentTeam==0? -GameActivity.SCREEN_WIDTH:GameActivity.SCREEN_WIDTH;
        final AtomicInteger counter = new AtomicInteger(6);
		for(int i=0; i<6; ++i) {
			Member member = mMemberList[currentTeam][i];
			if(member != null) {
				float x = Constants.OFFSET_X + i * (88 + 1);
				member.registerEntityModifier(new MoveXModifier(0.6f, x+offset, x) {
					protected void onModifierFinished(IEntity pItem) {
						if(counter.decrementAndGet() == 0) {
    ...
    private void changePlayerTurn(PlaygroundGameScene scene) {
    	scene.onPlayerTurn();
    ...
    private void playerTurn(PadContext context) {
        ...
        if(first || (scene instanceof  MultiplePadGameScene && ((MultiplePadGameScene)scene).playerAttacked)||
                (scene instanceof  MultiplePadGameScene7x6 &&  ((MultiplePadGameScene7x6)scene).playerAttacked)) {
            changePlayer(scene);
        } else {
        	changePlayerTurn(scene);
        }
    ...
    private void changePlayer(final PlaygroundGameScene scene) {
    	changing = true;
		if(!first) {
			final AtomicInteger counter = new AtomicInteger(6);
			int offset = currentTeam==0? -GameActivity.SCREEN_WIDTH:GameActivity.SCREEN_WIDTH;
			for(int i=0; i<6; ++i) {
				Member member = mMemberList[currentTeam][i];
				if(member != null) {
					member.registerEntityModifier(new MoveXModifier(.6f, member.sprite.getX(), member.sprite.getX()+offset) {
						protected void onModifierFinished(IEntity pItem) {
							if(counter.decrementAndGet() == 0) {
								displayNextTeam(scene);
							}
						};
					});
				} else {
    ...
    private void attackPhase(PadContext context) {
        ...
        for(int i=0; i<6; ++i) {
        	if (mMemberList[currentTeam][i] != null) {
        		bindStatus.add(mMemberList[currentTeam][i].isBinded());
        	} else {
...
//            Message.obtain(handler, ExecutorHandler.MSG_START_ATTACK, damage).sendToTarget();
    ...
    private void useMagicStone(final PadContext context) {
        ...
        awokenRecover(99);
    	removeNegative();
    	recoverPlayerHp(mTeamData.getHp());
    	getScene().clearBoard(new ICastCallback() {
    ...
    public <T extends Skill> void fireGlobalSkill(String key, T skill, final ICastCallback callback) {//String key,
    	...
    	int y = (int)(mMemberList[0][0].sprite.getY()-50);
    ...
    private <T extends Skill> void switchMemberSkill(T skill) {
    	...
    	if(index == 5) {
    		// other team's leader, don't switch back
    		// but skill is used, fix it someday? :P
    		return ;
    	}
    	if(index == 0) {
    		if(prevIndex[currentTeam] != -1) {
    			switchMember(prevIndex[currentTeam], true);
    			prevIndex[currentTeam] = -1;
    		}
    	} else {
    		switchMember(index, true);
    		prevIndex[currentTeam] = index;
    	}
    }

    private void switchMember(int index, boolean isSkill) {
        Member member = mMemberList[currentTeam][index];
        int another = (currentTeam==0)? 1:0;

        mMemberList[currentTeam][index] = mMemberList[currentTeam][0];
        mMemberList[currentTeam][0] = member;
        int normalIndex = 1;
        ...
        float normalY = mMemberList[currentTeam][normalIndex].sprite.getY();
        if(mMemberList[currentTeam][normalIndex].isSkillUsable()) {
            normalY = mMemberList[currentTeam][normalIndex].sprite.getY() + 10;
        }

        float n0y, n1y;
        if(!isSkill) {
            n0y = normalY;
            if(mMemberList[currentTeam][0].isSkillUsable()) {
                n0y -= 10;
            }
            n1y = normalY;
            if(mMemberList[currentTeam][index].isSkillUsable()) {
                n1y -= 10;
            }
        } else {
            n0y = mMemberList[currentTeam][0].sprite.getY();
            n1y = mMemberList[currentTeam][index].sprite.getY();
        }

        mMemberList[currentTeam][0].setPosition(mMemberList[currentTeam][index].sprite.getX(), n0y);
        mMemberList[currentTeam][index].setPosition(x, n1y);
        mMemberList[another][5] = mMemberList[currentTeam][0];
...
//		mMemberList[currentTeam][0] = member;
//		float x = member.sprite.getX();
//		float y = member.sprite.getY();
//
//		boolean isUsable1 = member.isSkillUsable();
//		boolean isUsable0 = mMemberList[currentTeam][index].isSkillUsable();
//		float addition0 = 0, addition1 = 0;
//		if(isUsable0 != isUsable1) {
//			if(isUsable0) {
//				addition0 = 10;
//				addition1 = -10;
//			} else {
...
//			}
//		}
//		mMemberList[currentTeam][0].setPosition(mMemberList[currentTeam][index].sprite.getX(), mMemberList[currentTeam][index].sprite.getY()+addition0);
//		mMemberList[currentTeam][index].setPosition(x, y+addition1);
//
//		mTeamData.switchMember(index);
//		getScene().updateSpecialLST();
//		setCurrentHp(mTeamData.getCurrentHp(), false);
    ...
    public void executeLeaderSwitch(ActiveSkill skill, final ICastCallback callback) {
    	if(!hasAlreadySwitched()) {
    		...
    		mLeaderSwitch.which = currentTeam;
    		int index = skill.getData().get(1);
    		Sprite sprite = mMemberList[currentTeam][index].sprite;
    ...
    public boolean hasAlreadySwitched() {
    	return mLeaderSwitch != null || (prevIndex[0] != -1 || prevIndex[1] != -1);
    }
    ...
    public boolean resistBadEffect(AwokenSkill awoken) {
		...
		if(!awokenLocked) {
			for(int j=0; j<2; ++j) {
				for(int i=0; i<5; ++i) {
					if(mMemberList[j][i] != null && !mMemberList[j][i].isBinded()) {
						resistant += getTargetAwokenCount(mMemberList[j][i].info,awoken);
					}
				}
			}
		}
    ...
    public void lockSkill(Enemy enemy, final int turns, final ICastCallback callback) {
        ...
        float toY = mMemberList[0][0].center().second;
    ...
    public void bindPets(Enemy enemy, List<Pair<Integer, Integer>> bindList, final ICastCallback callback) {
    	...
    	for(Pair<Integer, Integer> pair : bindList) {
            ...
            Pair<Float, Float> memberPos = mMemberList[currentTeam][pair.first].center();
            final MoveModifier modifier = new MoveModifier(0.25f, center.first, memberPos.first, center.second, memberPos.second){
            	...
            	protected void onModifierFinished(IEntity pItem) {
                    mSceneRef.get().engine.runOnUpdateThread(new Runnable() {
                        ...
                        @Override
                        public void run() {
                    		boolean binded = mMemberList[currentTeam][bindTo.first].bind(bindTo.second);
                    		if (binded) {
    ...
    private void showToast(final String text, final float offsetY) {
		...
		activity.runOnUiThread(new Runnable() {
			...
			@Override
			public void run() {
				TextView view = (TextView) customToast.getView().findViewById(android.R.id.text1);
				view.setText(text);
	        		...
	        		mTeamData.reduceDamage(enemy, recalcDamage, new IDamageCallback() {
						...
						public void onResult(double damage, List<Boolean> reduced) {
					    	...
					    	for(int i=0; i<6; ++i) {
					    		...
					    		if(fired) {
					    			Jump.apply(mMemberList[currentTeam][i].sprite, 0.3f);
					    		}
					    	}
...
//    }
    ...
    private void setCurrentHp(int currentHp, boolean withAnimation) {
        int hp = mTeamData.getHp();
        if(currentHp < 0) {
            currentHp = 0;
        } else if (currentHp > hp) {
            currentHp = hp;
        }
        mHp.setHp(currentHp, mTeamData.getHp(), withAnimation);
				...
				public void call(Integer arg0) {
					DialogUtil.getSkillDialog(getScene().getActivity(), info, counter, cd1, cd2,
							new DialogInterface.OnClickListener() {
								...
								public void onClick(DialogInterface dialog, int which) {
									dialog.dismiss();
									callback.onFire(MultiplePadEnvironment.this, (cd2>0 && counter>=(cd1+cd2))? 2:1);
								}
							},
							new DialogInterface.OnClickListener() {
			...
			.subscribe(new Action1<List<Pair<Boolean, Integer>>>() {
				...
				public void call(List<Pair<Boolean, Integer>> boosted) {
					...
					if(countDown.get() == 0) {
						...
						for(int index=0; index<6; ++index) {
							if(boosted.get(index).first && mMemberList[currentTeam][index] != null) {
								float x = mMemberList[currentTeam][index].sprite.getX();
								float y = mMemberList[currentTeam][index].sprite.getY();
								float w = mMemberList[currentTeam][index].sprite.getWidth() / 2f;
			...
			.subscribe(new Action1<List<Pair<Boolean, Integer>>>() {
				...
				public void call(List<Pair<Boolean, Integer>> boosted) {
					...
					for(int index=0; index<6; ++index) {
						...
						if(pair.first && mMemberList[currentTeam][index] != null) {
							float x = mMemberList[currentTeam][index].sprite.getX();
							float y = mMemberList[currentTeam][index].sprite.getY();
							float w = mMemberList[currentTeam][index].sprite.getWidth() / 2f;
